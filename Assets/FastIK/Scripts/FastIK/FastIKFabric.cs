...
namespace DitzelGames.FastIK
{
    ...
    public class FastIKFabric : MonoBehaviour
    {
        ...
        private void ResolveIK()
        {
            ...
            var targetD = targetPosition - GetPositionRootSpace(Bones[0]);

            //1st is possible to reach?
            if (targetD.sqrMagnitude >= CompleteLength * CompleteLength)
            {
                //just strech it
                var direction = (targetPosition - Positions[0]).normalized;
                var stretchAmt = Mathf.Min(targetD.magnitude / CompleteLength, 1 + StretchPercentage);
                //set everything after root
                for (int i = 1; i < Positions.Length; i++) {
                    //var boneStretch = stretchAmt * (BonesLength[i - 1] / CompleteLength);
                    Positions[i] = Positions[i - 1] + direction * BonesLength[i - 1] * (stretchAmt);
                }
            }
            else
            {
