...
namespace AwesomeTechnologies.Utility
{
    public class PolygonUtility{
        ...
        public static void AlignPointsWithTerrain(List<Vector3> pointList, bool closePolygon)
        {
            for (int i = 0; i <= pointList.Count - 1; i++)
            {
                ...
                {
                    if (!(hits[j].collider is TerrainCollider)) continue;
        ...
        public static List<UnityEngine.Vector2> DouglasPeucker(List<UnityEngine.Vector2> points, int startIndex, int lastIndex, float epsilon)
        {
            float dmax = 0f;
            ...
            if (dmax > epsilon)
            {
                ...
                var finalRes = new List<UnityEngine.Vector2>();
                for (int i = 0; i < res1.Count - 1; ++i)
                {
                    finalRes.Add(res1[i]);
                }

                foreach (UnityEngine.Vector2 t in res2)
                {
                    finalRes.Add(t);
                }
            ...
            {
                return new List<UnityEngine.Vector2>(new[] { points[startIndex], points[lastIndex] });
            }
        }

        public static float PointLineDistance(UnityEngine.Vector2 point, UnityEngine.Vector2 start, UnityEngine.Vector2 end)
        {
            if (start == end)
            {
                return UnityEngine.Vector2.Distance(point, start);
            }
